...
# Allow importing sibling packages when running this script directly
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))

import logging as _logging
_logging.basicConfig(level=_logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
logger = _logging.getLogger("DrugRepurposing")

#from is_action_space.action_space import (
...
try:
    ...
    logger.warning(f"Could not import is_action_space.action_space (will use mock fallbacks if available): {e}")

    # Fallbacks that use mock helpers when USE_MOCK_DATA is true,
    # otherwise surface a clear runtime error when code attempts live calls.
    def query_chembl(gene: str, max_compounds: int = 20):
        if USE_MOCK_DATA:
            return get_mock_chembl(gene), "CHEMBL_TARGET_MOCK"
        raise RuntimeError(f"query_chembl unavailable: {e}")

    def query_opentargets(gene: str):
        if USE_MOCK_DATA:
            return get_mock_ot(gene)
        raise RuntimeError(f"query_opentargets unavailable: {e}")

    def query_pubmed(term: str, max_results: int = 5):
        if USE_MOCK_DATA:
            return get_mock_pubmed(term)
        raise RuntimeError(f"query_pubmed unavailable: {e}")

    def query_faers(drug_name: str):
        if USE_MOCK_DATA:
            return get_mock_faers(drug_name)
        return {"total_events": 0, "serious_events": 0, "top_reactions": []}

    def query_fda_labels(drug_name: str):
        if USE_MOCK_DATA:
...
class Provenance:
    chembl_id: str = ""
    opentargets_id: str = ""
    chembl_target_id: str = ""    # <-- added to capture target id from ChEMBL
...
class ActionPlanningAgent(BaseAgent):
    ...
    Goal: Generate a structured 4-step plan for the workflow using the LLM.
    """
    ...
    def create_plan(self, target: str, disease: str) -> ActionPlan:
        ...
        # =====================================================================
        prompt = f"""
        You are an expert in drug repurposing. Create a structured action plan for the following task:
        a) Context: We want to find existing drugs that can be repurposed to target {target} for treating a disease {disease}.
        b) Return a JSON object with this required structure:
            {{"objective": "clear statement of the goal",
            "steps": [
                {{"step": 1, "action": "Query databases for compounds", "task_type": "data_mining"}},
                {{"step": 2, "action": "Enrich candidates with literature and safety data", "task_type": "enrichment"}},
                {{"step": 3, "action": "Score and rank candidates", "task_type": "scoring"}},
                {{"step": 4, "action": "Generate validation roadmap", "task_type": "roadmap"}}
            ]
            }}
        c) Guidelines:
        - The objective should be a concise statement of the overall goal.
        - Each step should have a clear action description and a task_type that matches one of the following: "data_mining", "enrichment", "scoring", "roadmap".
        - The steps should be logically ordered to achieve the objective.
        - Falls back gracefully if JSON unparseable.
        """

        raw = self._call_llm_json(prompt, fallback={ "objective": f"Find repurposable drugs for {target} in {disease}",
                                                      "steps": [
                                                          {"step": 1, "action": "Query databases for compounds", "task_type": "data_mining"},
                                                          {"step": 2, "action": "Enrich candidates with literature and safety data", "task_type": "enrichment"},
                                                          {"step": 3, "action": "Score and rank candidates", "task_type": "scoring"},
...
class RoutingAgent(BaseAgent):
    ...
    def route_enrichment_strategy(self, candidate: DrugCandidate, target: str, disease: str) -> RoutingDecision:
        ...
        Goal: Use the LLM to choose per-candidate strategies for literature + safety enrichment.
        """
        ...
        # =====================================================================
        prompt = f"""
        You are a routing agent deciding the best enrichment strategy for a drug candidate.
        Provide a JSON output selecting literature and safety strategies based on the candidate properties and context.
        ...
        - pchembl_value: {candidate.pchembl_value}
        ...
        Decision rules (use these to choose strategies):
        - literature_strategy: one of "target_focused", "disease_focused", "broad_search"
          * Prefer "target_focused" when mechanism_of_action is known and maps to the target.
          * Use "disease_focused" when MOA is unknown or evidence links drug to disease rather than target.
          * Use "broad_search" when exploratory searches are needed.
        - safety_strategy: one of "comprehensive", "faers_only", "basic"
          * Use "comprehensive" for late-stage drugs (max_phase 3-4).
          * Use "faers_only" for early-stage or limited clinical exposure (max_phase 0-2).
          * Use "basic" when safety data is sparse.

        Return JSON with keys:
        {{
          "literature_strategy": "target_focused" or "disease_focused" or "broad_search",
          "literature_reasoning": "why this strategy",
          "safety_strategy": "comprehensive" or "faers_only" or "basic",
          "safety_reasoning": "why this depth"
        }}

        Guidelines: 
        - Returns parseable JSON or fallback engages without crashing.
        """
...
class DataMiningAgent(BaseAgent):
    ...
    Goal: Implement the sequential workflow pattern: ChEMBL → Open Targets → Merge.
    This agent demonstrates sequential workflows where each step depends on the previous step's output.
    The pattern is: Step 1 (ChEMBL) -> Step 2 (Open Targets) -> Step 3 (Merge)
    """
    ...
    def run(self, target: str, disease: str) -> Tuple[List[DrugCandidate], str]:
        ...
        # =====================================================================
        # DataMiningAgent executes in order: ChEMBL query → Open Targets query → Merge.
        # Step 1: Query ChEMBL
        chembl_compounds, chembl_target_id = query_chembl(target, max_compounds=MAX_CHEMBL_COMPOUNDS)
        logger.info(f"  Step 1/3: Found {len(chembl_compounds)} compounds from ChEMBL")

        # Step 2: Query Open Targets
        ot_data = query_opentargets(target) or {"target_id": "", "known_drugs": []}
        logger.info(f"  Step 2/3: Found {len(ot_data.get('known_drugs', []))} drugs from Open Targets")

        # Step 3: Merge and sort
        candidates = self._merge_sources(chembl_compounds, ot_data, chembl_target_id, disease)
        candidates.sort(key=lambda c: (c.pchembl_value, c.max_phase), reverse=True)
        logger.info(f"  Step 3/3: Merged to {len(candidates)} unique candidates")
...
class LiteratureAgent(BaseAgent):
    ...
    This agent executes a PubMed search based on the routed strategy.
    Goal: Implement prompt chaining pattern: retrieve PubMed results, then assess them with the LLM; and have the LLM judge whether the retrieved articles support repurposing.
    This agent demonstrates prompt chaining where:
    - Chain Step 1: Search PubMed to get valid names (data retrieval), build PubMed query strings (drug + target, drug + disease, and a broad search where drug+target and drug+disease can be tried with related disease terms), retrieve article metadata, deduplicate by PMID, and save provenance.
    - Chain Step 2: Assess results with LLM (analysis), and Write the assessment results back to candidate fields (support level, strength, summary, supporting PMIDs).
    ...
    def enrich(self, candidate: DrugCandidate, target: str, disease: str,
        ...
        drug = search_names[0] if search_names else candidate.name

        if not search_names:
            candidate.provenance.pubmed_queries_tried = []
            candidate.literature_support_level = "none"
            candidate.literature_strength = 0
            candidate.literature_summary = f"No literature evidence for {candidate.chembl_id or candidate.opentargets_id}"
        ...
        if strategy == "target_focused":
            queries.append(f'"{search_names[0]}"[Title/Abstract] AND "{target}"[Title/Abstract]')
        elif strategy == "disease_focused":
            queries.append(f'"{search_names[0]}"[Title/Abstract] AND "{disease}"[Title/Abstract]')
        elif strategy == "broad_search":
            queries.append(f'"{search_names[0]}"[Title/Abstract] AND "{target}"[Title/Abstract]')
            queries.append(f'"{search_names[0]}"[Title/Abstract] AND "{disease}"[Title/Abstract]')
            queries.append(f'"{search_names[0]}"[Title/Abstract] AND ("{target}"[Title/Abstract] OR "{disease}"[Title/Abstract])')
    ...
    def _assess_articles(self, articles: List[Dict], drug: str, target: str,
        ...
        # =====================================================================
        prompt = f"""
        Ask for an assessment of the literature evidence regarding repurposing the drug for the target and disease.
        
        Context includes:
        - drug={drug}, 
        - target={target}, 
        - disease={disease}
        - ARTICLES information: {articles_text}
        
        Returns parseable JSON (or fallback engages) with keys:
...
class EvaluationAgent(BaseAgent):
    ...
    def score_and_rank(self, candidates: List[DrugCandidate], target: str,
        ...
        Goal: Implement an iterative ranking loop with persistent adjustments.
        ...
        - Initial scoring occurs only once; subsequent iterations recalculate totals from adjusted component scores without resetting.
        """
        ...
        for iteration in range(max_iterations):
            logger.info(f"EvaluationAgent: Starting iteration {iteration + 1}...")
        
            # 1. Score (first iteration) or recalculate totals (subsequent)
            if iteration == 0:
                self._score_candidates(enriched_candidates)
            else:
                for c in enriched_candidates:
                    c.total_score = sum(c.scores.get(k, 0) * v for k, v in self.WEIGHTS.items())
#                    c.total_score = max(c.total_score, 0)  # clamp to 0
#                    c.total_score = min(c.total_score, 10)  # clamp to 10

            # 2. Sort by total_score
            enriched_candidates.sort(key=lambda x: x.total_score, reverse=True)
            ...
            # 4. Check if approved
            if "appropriate" in critique.lower():
                logger.info("EvaluationAgent: Rankings approved.")
                break
            ...
            iteration_record = {
                ...
                "rankings": [{"name": c.name, "score": c.total_score, "scores": c.scores} for c in top_candidates],
                "critique": critique,
                "adjustments": adjustments,
                "approved": "appropriate" in critique.lower()
            }
            ranking_history.append(iteration_record)
    ...
    def _score_candidates(self, candidates: List[DrugCandidate]) -> List[DrugCandidate]:
        ...
        Goal: Compute component scores and weighted totals.
        ...
        for c in candidates:
            ...
            if c.pchembl_value >= 8.0:
                c.scores["target_binding"] = 10
            elif c.pchembl_value >= 7.0:
                c.scores["target_binding"] = 8
            elif c.pchembl_value >= 6.0:
                c.scores["target_binding"] = 6
            elif c.pchembl_value > 0:
                c.scores["target_binding"] = 4
            else:
                c.scores["target_binding"] = 2

            # literature_support score
            c.literature_strength = c.literature_strength or 0
            c.scores["literature_support"] = max(0, min(10, c.literature_strength))
            
            # safety_profile score 
            if "BOXED_WARNING" in c.safety_flags:
                c.scores["safety_profile"] = 3
            elif "HIGH_SERIOUS_RATIO" in c.safety_flags:
                c.scores["safety_profile"] = 5
            elif c.faers_event_count > 0:
                c.scores["safety_profile"] = 7
            else:
                c.scores["safety_profile"] = 5

            # development_stage score
            if c.max_phase == 4:
                c.scores["development_stage"] = 10
            elif c.max_phase == 3:
                c.scores["development_stage"] = 8
            elif c.max_phase == 2:
                c.scores["development_stage"] = 6
            elif c.max_phase == 1:
                c.scores["development_stage"] = 4
            else:
                c.scores["development_stage"] = 2

            # disease_relevance score
            c.scores["disease_relevance"] = c.disease_relevance_score * 10

            # Calculate total_score
            c.total_score = sum(c.scores[component] * self.WEIGHTS[component] for component in self.WEIGHTS)
...
class DrugRepurposingOrchestrator:
    ...
    def _execute_step(self, task_type: str, max_candidates: int) -> Any:
        ...
        Goal: Integrate all agents into the end-to-end workflow.
        ...
        if task_type == "data_mining":
           # Mine databases (unpack tuple: (candidates_list, chembl_target_id))
            ...
            for c in top:
                ...
                # 3. Store strategies on candidate
                c.literature_strategy = routing.inputs["literature_strategy"]
                c.safety_strategy = routing.inputs["safety_strategy"]
                # 4. Literature enrichment
                self.literature.enrich(c, self.target, self.disease, c.literature_strategy)
                # 5. Safety evaluation
                self.safety.evaluate(c, c.safety_strategy)
                # 6. Mark enriched and timestamp
                c.is_enriched = True
                c.enrichment_timestamp = datetime.now()
            ...
            # Score and rank
            # Only candidates marked is_enriched are included in final scoring
            candidates = [c for c in self.candidates if c.is_enriched]
            candidates, ranking_history = self.evaluator.score_and_rank(candidates, self.target, self.disease)
            self.candidates = candidates  # To update with scored candidates
            return candidates, ranking_history

        elif task_type == "roadmap":
            # Generate roadmap
            print("  Generating validation roadmap...")
            self.validation_roadmap = self.evaluator.generate_roadmap(self.candidates, self.target, self.disease)
            return self.validation_roadmap

        return print(f"  No implementation for task type: {task_type}")
        # =====================================================================
        # END TODO 10
        # =====================================================================
...
def export_results(result: RepurposingResult, output_dir: str = "."):
    ...
    for i, c in enumerate(result.candidates, 1):
        # Build ranking history snapshot (use recorded ranking_history, not undefined 'r')
        ranking_hist = []
        for rec in result.ranking_history:
            ranking_hist.append({
        ...
        rows.append({
            ...
            "audit_routing_decision_id": next((d["decision_id"] for d in result.audit_log.routing_decisions if d["candidate"] == c.name), None),
            "audit_enrichment_timestamp": c.enrichment_timestamp.isoformat() if c.enrichment_timestamp else None,
            "audit_provenance": {
                "faers_query_matched": c.provenance.faers_query_matched,
                "label_found": getattr(c, "label_found", False),
                "has_boxed_warning": getattr(c, "has_boxed_warning", False)
            },
